...
class App extends Component {
  ...
  render() {
    ...
    const navStyle = { borderTop: smallScreen && showMenu ? '1px solid #e8e8e8' : 'none' };
    return (
      <div className="App">
              ...
              </div>
              {
                showMenu &&
                  <div style={{ display: 'flex', flexDirection: smallScreen ? 'column' : 'row' }}>
                    <div className="nav-link" onClick={ () => this.onNavClick(this.services) } style={ navStyle }>
                      <a>services</a>
                    </div>
                    <div className="nav-link" onClick={ () => this.onNavClick(this.why) } style={ navStyle }>
                      <a>why us?</a>
                    </div>
                    <div className="nav-link" onClick={ () => this.onNavClick(this.testimonials) } style={ navStyle }>
                      <a>testimonials</a>
                    </div>
                    <div className="nav-link" onClick={ () => this.onNavClick(this.contact) } style={ navStyle }>
                      <a>contact us</a>
                    </div>
                  </div>
              }
        ...
        <main role="main" style={{ paddingTop: '50px' }}>
          ...
          <PageSection ref={(section) => { this.home = section }}>
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100%', color: '#01149b', fontFamily: "'Merriweather', serif" }}>
              ...
              <h1 style={{ marginTop: '-1px', padding: '10px 20px', borderBottom: '2px solid #01149b', borderTop: '2px solid #01149b' }}>J<span style={{ fontSize: '.8em' }}>&</span>S McGrath</h1>
              <div>
                <p>Irish haulage since 1950</p>
              </div>
            </div>
          </PageSection>
